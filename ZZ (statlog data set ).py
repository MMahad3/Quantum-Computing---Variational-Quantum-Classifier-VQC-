...
def my_ansatz(num_qubits, reps):
    ...
    parameters = ParameterVector('θ', length=num_qubits * reps * 2)  
...
X = np.array(statlog_heart.data.features)  # Ensuring features are numpy arrays
y = np.array(statlog_heart.data.targets)   # Ensuring targets are numpy arrays
